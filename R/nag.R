...
glyph_to_nag <- function(glyph) {
  ...
  if (is.na(glyph)) return(NULL)
  if (glyph == "\u2212+") return(chess_pgn$NAG_BLACK_DECISIVE_ADVANTAGE)
  if (glyph == "-+") return(chess_pgn$NAG_BLACK_DECISIVE_ADVANTAGE)
  if (glyph == "\u2213") return(chess_pgn$NAG_BLACK_MODERATE_ADVANTAGE)
  if (glyph == "\u21c6") return(chess_pgn$NAG_BLACK_MODERATE_COUNTERPLAY)
  if (glyph == "\u2a01") return(chess_pgn$NAG_BLACK_SEVERE_TIME_PRESSURE)
  if (glyph == "\u2a71") return(chess_pgn$NAG_BLACK_SLIGHT_ADVANTAGE)
  if (glyph == "\u2a00") return(chess_pgn$NAG_BLACK_ZUGZWANG)
  if (glyph == "??") return(chess_pgn$NAG_BLUNDER)
  if (glyph == "!!") return(chess_pgn$NAG_BRILLIANT_MOVE)
  if (glyph == "=") return(chess_pgn$NAG_DRAWISH_POSITION)
  if (glyph == "?!") return(chess_pgn$NAG_DUBIOUS_MOVE)
  if (glyph == "\u25a1") return(chess_pgn$NAG_FORCED_MOVE)
  if (glyph == "!") return(chess_pgn$NAG_GOOD_MOVE)
  if (glyph == "?") return(chess_pgn$NAG_MISTAKE)
  if (glyph == "!?") return(chess_pgn$NAG_SPECULATIVE_MOVE)
  if (glyph == "\u221e") return(chess_pgn$NAG_UNCLEAR_POSITION)
  if (glyph == "+\u2212") return(chess_pgn$NAG_WHITE_DECISIVE_ADVANTAGE)
  if (glyph == "+-") return(chess_pgn$NAG_WHITE_DECISIVE_ADVANTAGE)
  if (glyph == "\u00b1") return(chess_pgn$NAG_WHITE_MODERATE_ADVANTAGE)
  if (glyph == "\u21c6") return(chess_pgn$NAG_WHITE_MODERATE_COUNTERPLAY)
  if (glyph == "\u2a01") return(chess_pgn$NAG_WHITE_SEVERE_TIME_PRESSURE)
  if (glyph == "\u2a72") return(chess_pgn$NAG_WHITE_SLIGHT_ADVANTAGE)
  if (glyph == "\u2a00") return(chess_pgn$NAG_WHITE_ZUGZWANG)
}
